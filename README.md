...
```bash
uv pip install -e '.[foundationpose]'
...
filename is `deployable_foundation_stereo_s_dynamic_v2.0.onnx`. Once SAM3 and FoundationPose
artifacts are ready, a run can compile stereo automatically:

```bash
python script/run_pipeline.py --config <profile> --dataset <dataset> --max-scenes 2
```

The runtime compiles a static profile from the actual padded stereo input shape on the first
cache miss. Another shape can require another plan. First-use stereo compilation can take a
long time; prebuilding remains useful for development:
...
different cache keys. Fixed plans resize by width and pad height; excess height raises an error
instead of cropping. Rebuild a suitable profile when the rig or fitting requirements change.

### 2.5 Export and build SAM3

The checkout/checkpoint in §2.2 are needed for export, not TensorRT inference. ONNX is already
a declared dependency. Export into the configured model directory:

```bash
python tools/export_sam3_to_onnx.py --config <profile> --checkpoint /path/to/sam3.pt --device cuda
```

`--output-dir /path/to/models` overrides the export destination. If using that flag, configure
the runtime to use the same directory. CPU export is also supported with `--device cpu`.
...
Use `--device cuda:0 --fp-device-id 0` for the single-GPU pipeline. ONNX checking runs during
export. Optional `--verify` compares each wrapper to ONNX Runtime on CPU (install
`onnxruntime` in the export environment first). This is separate from validating TensorRT
outputs on real images.
