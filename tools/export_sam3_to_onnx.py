...
def export_component(wrapper, inputs, path, input_names, output_names, verify=False, skip_existing=False):
    ...
    # Constant position encodings may be folded into a decoder. Other inputs must survive.
    required = set(input_names) - {"pos2"}
    if not required <= actual_inputs:
        raise ValueError(f"{path.name}: missing exported inputs {required - actual_inputs}")
    actual = session.run(output_names, {name: value for name, value in feed.items() if name in actual_inputs})
    for name, reference, value in zip(output_names, expected, actual, strict=True):
        np.testing.assert_allclose(value, reference.detach().cpu().numpy(), rtol=VERIFY_RTOL, atol=VERIFY_ATOL,
...
def export_sam3_models(
    checkpoint_path: Path, output_dir: Path, device: str = "cuda", verify: bool = False, skip_existing: bool = False
):
    import sam3
    ...
    with patch.object(builder, "_create_vit_backbone", partial(builder._create_vit_backbone, use_rope_real=True)), \
        ...
        export = partial(export_component, verify=verify, skip_existing=skip_existing)
...
def main():
    ...
    parser.add_argument("--device", choices=["cpu", "cuda"], default=default_dev)
    parser.add_argument("--skip-existing", action="store_true", help="Skip re-exporting existing ONNX files")
    parser.add_argument("--verify", action="store_true", help="Compare each ONNX graph to PyTorch using ONNX Runtime (CPU)")
