...
pipeline/
  ...
  src/foundationpose_perception_pipeline/   the importable package
    ...
    runtime.py               optional torch/device helpers and NumPy conversion
...
`regex` splitting. Stereo retains ImageNet normalization, rectification, disparity scaling and
...
| `foundationpose_engine_cache/` | FoundationPose's TensorRT engines managed by the FoundationPose SDK and built on first use. Separate from the stereo/SAM3 cache beneath models_dir. |
