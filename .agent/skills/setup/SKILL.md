---
name: setup
description: Installs and verifies the perception pipeline environment, native TensorRT stereo/SAM3 runtime, SAM3 ONNX export tools, and the TAO FoundationPose SDK. Use for setup, provisioning, missing dependencies or CUDA/TensorRT library failures.
---
...
Runtime needs the pipeline environment, compatible NVIDIA GPU/runtime, FoundationPose SDK,
...
The extra supplies TensorRT, CUDA runtime and `cuda-bindings`. ONNX and `regex`
are project dependencies. Torch remains installed although stereo/SAM3 inference uses TRT.
...
Optional exporter `--verify` needs ONNX Runtime and compares wrappers on CPU; it is not a
TensorRT accuracy gate.

## 4. FoundationPose SDK
